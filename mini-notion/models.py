...
class Usuario(Base): #Classe do usuario
    ...
    # Token de sessão simples: gerado no /login, guardado aqui e exigido
    # (via header Authorization: Bearer <token>) em toda rota que precisa de login.
...
class Nota(Base):
    ...
    # Agora toda nota pertence a um usuário — sem isso não dá pra saber quem pode editar/ver
...
# Model novo: é a tabela que o front (index.html/script.js) espera em /notas.
# Não existia antes — por isso o front não conseguia salvar nada.
